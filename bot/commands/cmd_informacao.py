"""
bot/cmd_informacao.py — /informacao: adiciona um texto em varios chamados.

So existe no bot: nao ha tela no app desktop para isto (ver
services/flow_informacao_pw.py). Fica na mesma familia do /atendimento no
Assyst (Ações de relógio), mas executa NA HORA — nao tem agenda, nao tem
horario marcado, e o MESMO texto vale para todos os chamados da lista.
"""
import asyncio
import os
...
from bot.services import credencial_servico, informacao_service

# Preenche tudo e PARA antes de salvar. Mesmo padrao do REQUISICAO_TESTE: flag
# de ambiente, nao botao — o app desktop nao expoe isto.
INFORMACAO_TESTE = os.getenv("BOT_INFORMACAO_TESTE") == "1"
...
async def cmd_informacao(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.effective_chat.id
    if not liberado(update):
        return

    WIZARD[chat_id] = {"fluxo": "informacao", "passo": "chamados"}
    aviso = "\n\n⚠️ MODO TESTE ligado: nada será salvo." if INFORMACAO_TESTE else ""
...
async def responder(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if estado["passo"] == "texto":
        ...
        aviso = (
            ...
            else f"⚠️ Isto ADICIONA o texto abaixo em {len(chamados)} chamado(s).\n\n"
        )
        teclado = InlineKeyboardMarkup([[
            InlineKeyboardButton("✅ Confirmar", callback_data="in:ok"),
            InlineKeyboardButton("✖️ Cancelar", callback_data="in:nao"),
        ]])
        await update.message.reply_text(
            f"{aviso}Chamados:\n{lista}\n\nTexto:\n{texto}\n\nConfirma?",
...
async def confirmar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not estado or estado.get("passo") != "confirmar":
        await query.edit_message_text("Esse pedido expirou. Mande /informacao de novo.")
        return

    if query.data == "in:nao":
    ...
    chamados = estado["chamados"]
    texto = estado["texto"]
    await query.edit_message_text(f"▶️ Adicionando informação em {len(chamados)} chamado(s)...")
    ...
    try:
        resultados = await asyncio.to_thread(
            informacao_service.adicionar_lote,
            chamados, texto, matricula, senha, log, INFORMACAO_TESTE,
        )
    except Exception as e:
        log_bot.exception("Falha na informacao de %s", matricula)
