"""
bot/cmd_fornecedor.py — /fornecedor: registra 'Aguardando Info do Fornecedor'
em varios chamados.

So existe no bot: nao ha tela no app desktop para isto (ver
services/flow_fornecedor_pw.py). Irmao do /informacao — mesma wizard, mesma
execucao imediata com o MESMO texto pra todos os chamados da lista; só muda o
botao clicado no Assyst (Ações de relógio -> Aguardando Info do Fornecedor).
"""
import asyncio
import os
...
FORNECEDOR_TESTE = os.getenv("BOT_FORNECEDOR_TESTE") == "1"
...
async def cmd_fornecedor(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.effective_chat.id
    if not liberado(update):
        return

    WIZARD[chat_id] = {"fluxo": "fornecedor", "passo": "chamados"}
    aviso = "\n\n⚠️ MODO TESTE ligado: nada será salvo." if FORNECEDOR_TESTE else ""
...
async def responder(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if estado["passo"] == "texto":
        ...
        aviso = (
            ...
            else f"⚠️ Isto marca 'Aguardando Info do Fornecedor' com o texto "
                 f"abaixo em {len(chamados)} chamado(s).\n\n"
        )
        teclado = InlineKeyboardMarkup([[
            InlineKeyboardButton("✅ Confirmar", callback_data="fn:ok"),
            InlineKeyboardButton("✖️ Cancelar", callback_data="fn:nao"),
        ]])
        await update.message.reply_text(
            f"{aviso}Chamados:\n{lista}\n\nTexto:\n{texto}\n\nConfirma?",
...
async def confirmar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not estado or estado.get("passo") != "confirmar":
        await query.edit_message_text("Esse pedido expirou. Mande /fornecedor de novo.")
        return

    if query.data == "fn:nao":
    ...
    texto = estado["texto"]
    await query.edit_message_text(
        f"▶️ Registrando 'Aguardando Info do Fornecedor' em {len(chamados)} chamado(s)..."
    )
    ...
    try:
        resultados = await asyncio.to_thread(
            fornecedor_service.aguardar_lote,
            chamados, texto, matricula, senha, log, FORNECEDOR_TESTE,
        )
    except Exception as e:
        log_bot.exception("Falha no fornecedor de %s", matricula)
