...
def flow_nomenclature(df, secretaria, log):
    log("Criando Requisição de Serviço...", tipo="status")

    time.sleep(2)
    pyautogui.PAUSE = 0.5

    description_father = f"Solicito validação e correção de nomenclatura nos micros da {secretaria}:\n\n"
    for _, row in df.iterrows():
        description_father += f" - {row['MARCA/MODELO']} | Tombo: {row['TOMBO ANTIGO']}/{row['TOMBO NOVO']} | Nome: {row['NOME']}\n"

    # Usuário afetado
    pyautogui.click(x=-1897, y=435)
    write('400566')
    ...
    write('Correção de Nomenclatura')
    ...
    for _, row in df.iterrows():
        description_son = (f"Solicito validação e correção de nomenclatura no micro da {secretaria}:\n\n"
